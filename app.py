...
def create_invoice():
    """API endpoint to create a Lightning invoice.

    Expects JSON body with either product_id or amount (sats). If product_id
    is provided it will use the price from PRODUCTS.
    Returns JSON: { payment_request, r_hash, qr_base64 }
    """
    try:
        ...
        if product_id:
            ...
            # prefer explicit amount if provided, otherwise use product price
            amount = amount or product.get("price")

        if not amount:
            return jsonify({"error": "amount is required"}), 400

        memo = f"Webstore: {product['name']}" if product else "Webstore invoice"
        result = lnd.add_invoice(amount=amount, memo=memo)

        payment_request = result.get("payment_request")
        r_hash = result.get("r_hash")

        # Generate QR code PNG (base64) from payment_request
        qr_base64 = generate_qr_base64(payment_request.upper()) if payment_request else ""
...
#Uyanda and Nhlanhla contributed to this code.
